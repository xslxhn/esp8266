...
void ICACHE_FLASH_ATTR my_station_init(struct ip_addr *remote_ip,
	...
	//连接服务器
	uint8_t res=espconn_connect(&user_tcp_conn);
	printf("espconn_connect :%d\r\n",res);
}

void ICACHE_FLASH_ATTR my_station_disinit(void)
{
	if(0==espconn_delete(&user_tcp_conn))
	//if(0==espconn_disconnect(&user_tcp_conn))
	{
		printf("espconn_disconnect OK\r\n");
	}
	else
	{
		printf("espconn_disconnect ERR\r\n");
	}
